...
function iosPWASplash (icon, color = 'white') {
  ...
  const deviceWidth = screen.width // eslint-disable-line
  const deviceHeight = screen.height // eslint-disable-line
  ...
  const iconImage = new Image() // eslint-disable-line
  ...
  iconImage.onload = function () {
    ...
    const touchIconPictureSize = 150
    ...
    canvas3.height = touchIconSize
    canvas3.width = touchIconSize
    ctx.fillStyle = color
    ...
    const x3 = (canvas3.width - touchIconPictureSize) / 2
    const y3 = (canvas3.height - touchIconPictureSize) / 2
    ...
    ctx3.drawImage(iconImage, x3, y3, touchIconPictureSize, touchIconPictureSize)
    const imageDataURL = canvas.toDataURL('image/png')
    const imageDataURL2 = canvas2.toDataURL('image/png')
    const imageDataURL3 = canvas3.toDataURL('image/png')
    ...
    const appleTouchStartupImageLink3 = document.createElement('link')
    appleTouchStartupImageLink3.setAttribute('rel', 'apple-touch-icon')
    appleTouchStartupImageLink3.setAttribute('href', imageDataURL3)
    document.head.appendChild(appleTouchStartupImageLink3)
  }
}
